	...
	class BrandCo_Form {
		...
		public function create_form( $args ) {
			...
			if ( is_array( $args['fields'] ) ) {
				...
				// if ( $args['js'] === true ) {
					// echo '<form action="' . esc_url( $_SERVER['REQUEST_URI'] ) . '" method="post" onsubmit="submitForm("FormID--' . $FormTitle . '"); return false; ">';
				// } else {
				...
				if ( $args['js'] === true ) {
					echo '<button type="button" id="' . $FormTitle . '-submit-button" onclick=\'submitForm("FormID--' . $FormTitle . '")\'>' . $SubmitTitle . '</button>';	
				} else {
				...
				// echo '<script>window.onload = function(){ if (location.hash === "#FormID--' . $FormTitle . '") { location.href="#FormID--' . $FormTitle . '"; } }</script>';
